...
class Ghost():
    def __init__(self):
        self.srcMeshs = set()
        self.ghostColor = [128,128,128]
        self.baseTransparency = 1
    ...
    def UpdateBaseTranparency(self, newTransparency):      
        ...
        mc.setAttr(self.GetGhostGrpName() + ".base", newTransparency*100)
        self.UpdateGhostTransparency()

    def UpdateGhostTransparency(self):
        mc.setAttr(self.GetGhostGrpName() + ".range", self.transparencyRange)
        ...
        for ghost in allGhosts:
            ...
            if distance > self.transparencyRange:
                mc.setAttr(ghostMat + ".transparency", self.baseTransparency,self.baseTransparency,self.baseTransparency, type = "double3")
                continue
    ...
    def UpdateGhostColors(self, r, g, b):
        ...
        self.ghostColor[1] = g
        self.ghostColor[2] = b
        mc.setAttr(self.GetGhostGrpName() + ".red", r)
        mc.setAttr(self.GetGhostGrpName() + ".green", g)
        mc.setAttr(self.GetGhostGrpName() + ".blue", b)
        allGhosts = mc.listRelatives(self.GetGhostGrpName(), c=True)
        if not allGhosts:
            return
        for ghost in allGhosts:
    ...
    def InitSrcMeshFromGhostGrp(self):
        ...
        self.ghostColor[0] = mc.getAttr(self.GetGhostGrpName()+".red")
        self.ghostColor[1] = mc.getAttr(self.GetGhostGrpName()+".green")
        self.ghostColor[2] = mc.getAttr(self.GetGhostGrpName()+".blue")
        self.baseTranparency = mc.getAttr(self.GetGhostGrpName()+".base")/100
        self.transparencyRange = mc.getAttr(self.GetGhostGrpName()+".range")
        if not srcMeshAttr:
            return
    ...
    def InitGhostGrpIfNotExist(self):
        if not mc.objExists(self.GetGhostGrpName()):
            ...
            mc.addAttr(self.GetGhostGrpName(), ln = "red", at = "double")
            mc.addAttr(self.GetGhostGrpName(), ln = "green", at = "double")
            mc.addAttr(self.GetGhostGrpName(), ln = "blue", at = "double")
            mc.addAttr(self.GetGhostGrpName(), ln = "base", at = "double")
            mc.addAttr(self.GetGhostGrpName(), ln = "range", at = "double")
...
class ColorPicker(QWidget):
    ...
    def __init__(self, width = 95, height = 20):
        super().__init__()
...
class GhostWidget(QWidget):
    ...
    def CreateMatCtrlSection(self):
        ...
        self.ghostColorPicker.color.setRedF(self.ghost.ghostColor[0])
        self.ghostColorPicker.color.setGreenF(self.ghost.ghostColor[1])
        self.ghostColorPicker.color.setBlueF(self.ghost.ghostColor[2])
        ...
        transSlider.setValue(self.ghost.baseTranparency*100)
